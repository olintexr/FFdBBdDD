...
## 6. Representación visual del modelo E‑R

A continuación se presenta una vista más moderna del modelo, sin imágenes, usando Mermaid para resaltar las entidades, sus relaciones y la lógica del negocio.

```mermaid
flowchart LR
    CATEDRA["Cátedra<br/>id_catedra, nombre, descripción"] -->|tiene| CURSO["Curso<br/>id_curso, código, nombre, cupos_maximos"]
    PROFESOR["Profesor<br/>id_profesor, nombre, apellido, email"] -->|dicta| CURSO
    ALUMNO["Alumno<br/>id_alumno, cédula, nombre, apellido, email"] -->|se inscribe| CURSO
```
...
- Representamos el modelo con diagramas visuales compatibles con Markdown.  
