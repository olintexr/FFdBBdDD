...
## 6. Representación visual del modelo

### Supertipo y subtipos

```mermaid
flowchart TB
    PERSONA["PERSONA<br/>id_persona, nombre, apellidos, fecha_nacimiento, dirección, teléfono, correo_electrónico"]
    ALUMNO["ALUMNO<br/>fecha_ingreso, estatus"]
    EMPLEADO["EMPLEADO<br/>id_empleado, puesto, área, salario, fecha_contratación, tipo_empleo"]
...
```mermaid
flowchart LR
    ALUMNO["Alumno"] -->|pertenece a| SALON["Salón"]
    ALUMNO -.->|fecha_asignación, periodo| SALON
```
...
```mermaid
flowchart LR
    EMPLEADO["Empleado"] -->|asignado a| SALON["Salón"]
    EMPLEADO -.->|rol, horario, materia, grupo| SALON
```
...
flowchart TB
    ...
    ALUMNO["ALUMNO<br/>fecha_ingreso, estatus"]
    EMPLEADO["EMPLEADO<br/>id_empleado, puesto, área, salario, fecha_contratación, tipo_empleo"]
    SALON["SALÓN<br/>id_salon, nombre_salon, capacidad, ubicación"]

    PERSONA -->|es| ALUMNO
    PERSONA -->|es| EMPLEADO
    ALUMNO -->|pertenece a| SALON
    EMPLEADO -->|asignado a| SALON
```
...
- Representaciones visuales y Mermaid  
...
3. Representa en un diagrama Mermaid un modelo con herencia.  
...
- **Asunto:**  
  `TAREA 03 - CAPÍTULO 1 - APELLIDOS Y NOMBRES COMPLETOS DEL ALUMNO EN MAYÚSCULA`  
  Ejemplo:  
  `TAREA 03 - CAPÍTULO 1 - PEREZ BONALDE, JUAN ANTONIO`

- **Nombre del archivo:**  
  `TAREA 03 - CAPÍTULO 1 - APELLIDOS Y NOMBRES COMPLETOS DEL ALUMNO EN MAYÚSCULA.docx`  
  Ejemplo:  
  `TAREA 03 - CAPÍTULO 1 - PEREZ BONALDE, JUAN ANTONIO.docx`
